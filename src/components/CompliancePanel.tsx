  ...
  return (
    <div className="mt-6 border-t border-anb-line pt-5">
      <button
        ...
        className="inline-flex items-center gap-2 rounded-lg border border-anb-line px-3.5 py-2 text-xs font-semibold text-anb-navy transition-colors hover:border-slate-300 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:border-anb-line disabled:hover:bg-transparent"
      >
        {status === "loading" ? <IconSpinner /> : <IconShield />}
        {status === "loading"
          ? "جاري المراجعة..."
          : status === "success"
      ...
      {hasPanel && (
        <div className="anb-fade-in mt-4 rounded-xl border border-anb-line bg-slate-50/60 p-5">
          {status === "error" && (
            ...
            <div className="space-y-4">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-2xl font-bold tabular-nums text-anb-navy">
                    {result.score}
